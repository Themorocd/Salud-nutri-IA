...
export class AdminComponent implements OnInit {
  ...
  // Helper para obtener el token solo en navegador
...
eliminarObjetivo(id: number) {
  const token = this.getToken();
  const headers = new HttpHeaders().set('Authorization', `Bearer ${token}`);
  this.http.delete(`http://localhost:3000/api/admin/objetivos/${id}`, { headers })
    .subscribe(() => this.cargarObjetivos());
}
...
eliminarAlergia(id: number) {
  const token = this.getToken();
  const headers = new HttpHeaders().set('Authorization', `Bearer ${token}`);
  this.http.delete(`http://localhost:3000/api/admin/alergias/${id}`, { headers })
    .subscribe(() => this.cargarAlergias());
}

// Eliminar tipo de dieta
eliminarTipoDieta(id: number) {
  const token = this.getToken();
  const headers = new HttpHeaders().set('Authorization', `Bearer ${token}`);
  this.http.delete(`http://localhost:3000/api/admin/tipos-dieta/${id}`, { headers })
    .subscribe(() => this.cargarTiposDieta());
}
}
